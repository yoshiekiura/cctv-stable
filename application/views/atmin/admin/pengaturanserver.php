    ...
    <div class="row">
        <div class="col-md-6 m-b-20">
            ...
            <div class="btn-group g-otp col-12 m-lr-0 form-group m-b-10 col-md-6 m-b-30" role="group">
				...
				?>
                <button id="aktifotp" onclick="saveOTP(1)" type="button" style="border: 1px solid #bbb;"
                    class="col-6 btn btn-sm <?=$setaktif?>"><b>OTP</b></button>
                <button id="aktifmanual" onclick="saveOTP(0)" type="button" style="border: 1px solid #bbb;"
                    class="col-6 btn btn-sm <?=$setnonaktif?>"><b>MANUAL</b></button>
            </div>
            <div class="form-group titel" style="font-weight: bold;">
            ...
            <div class="form-group">
                ...
                <input type="password" name="email_password" class="form-control col-6" value="abcdefghijk1234567890"
                    <?=$read?> />
                <?php }else{ ?>
                <input type="password" name="email_password" class="form-control col-6"
                    value="<?=$set->email_password?>" />
                <?php } ?>
            </div>
            <div class="form-group">
                <label>Mail Server Domain</label>
                <input type="text" name="email_server" class="form-control col-10 col-md-8"
                    value="<?=$set->email_server?>" <?=$read?> />
            </div>
            <div class="form-group">
                <label>Mail Server Port</label>
                <input type="text" name="email_port" class="form-control col-6 col-md-3" value="<?=$set->email_port?>"
                    <?=$read?> />
            </div>
        </div>
        <div class="col-md-6 m-b-20">
            ...
            <div class="btn-group g-vendor col-12 m-lr-0 form-group m-b-10" role="group">
				...
				?>
                <button id="aktifwoowa" onclick="saveApiWasap('woowa')" type="button" style="border: 1px solid #bbb;"
                    class="col-4 btn btn-sm <?=$setaktif?>"><b>WOOWA</b></button>
                <button id="aktifwablas" onclick="saveApiWasap('wablas')" type="button" style="border: 1px solid #bbb;"
                    class="col-4 btn btn-sm <?=$setnonaktif?>"><b>WABLAS</b></button>
                <button id="aktifss" onclick="saveApiWasap('starsender')" type="button" style="border: 1px solid #bbb;"
                    class="col-4 btn btn-sm <?=$setss?>"><b>STARSENDER</b></button>
                <button id="aktifwagw" onclick="saveApiWasap('wagw')" type="button" style="border: 1px solid #bbb;"
                    class="col-4 btn btn-sm <?=$setwagw?>"><b>SERVER</b></button>
            </div>
            <div class="form-group woowa" <?php if($set->api_wasap != "woowa"){ echo 'style="display:none"'; } ?>>
                ...
                <?php if($this->func->demo() == true){ ?>
                <input type="text" name="woowa" class="form-control" value="abcdefghijklmnopqrstuvwxyz1234567890"
                    <?=$read?> />
                <?php }else{ ?>
                <input type="text" name="woowa" class="form-control" value="<?=$set->woowa?>" />
                <?php } ?>
                <small><i class="text-danger">kosongkan apabila ingin menonaktifkan notifikasi
                        Whatsapp</i></small><br />
            </div>
            <div class="form-group wablas" <?php if($set->api_wasap != "wablas"){ echo 'style="display:none"'; } ?>>
                ...
                <?php if($this->func->demo() == true){ ?>
                <input type="text" name="wablas" class="form-control" value="abcdefghijklmnopqrstuvwxyz1234567890"
                    <?=$read?> />
                <?php }else{ ?>
            ...
            <div class="form-group wablas" <?php if($set->api_wasap != "wablas"){ echo 'style="display:none"'; } ?>>
                ...
                <?php if($this->func->demo() == true){ ?>
                <input type="text" name="wablas_server" class="form-control" value="https://domain.wablas.com"
                    <?=$read?> />
                <?php }else{ ?>
            ...
            <div class="form-group ss" <?php if($set->api_wasap != "starsender"){ echo 'style="display:none"'; } ?>>
                <label>API Key <b>Star Sender</b> (<a href="https://starsender.id/"
                        target="_blank">starsender.id</a>)</label>
                <?php if($this->func->demo() == true){ ?>
                <input type="text" name="starsender" class="form-control" value="abcdefghijklmnopqrstuvwxyz1234567890"
                    <?=$read?> />
                <?php }else{ ?>
            ...
            <div class="form-group wagw" <?php if($set->api_wasap != "wagw"){ echo 'style="display:none"'; } ?>>
                <label>Nomor Sender (diawali dengan kode negara, contoh: 6281334327091)</label>
                <?php if($this->func->demo() == true){ ?>
                <input type="text" name="wagw_nomer" class="form-control" value="6281334327091" <?=$read?> />
                <?php }else{ ?>
                <input type="text" id="wagw_nomer" name="wagw_nomer" class="form-control"
                    value="<?=$set->wagw_nomer?>" />
                <?php } ?>
            </div>
            <div class="form-group wagw" <?php if($set->api_wasap != "wagw"){ echo 'style="display:none"'; } ?>>
                <label>Domain/Subdomain Server Whatsapp</label>
                <?php if($this->func->demo() == true){ ?>
                <input type="text" name="wagw_domain" class="form-control" value="https://whatsapp.cctvgrosir.com/"
                    <?=$read?> />
                <?php }else{ ?>
                <input type="text" id="wagw_domain" name="wagw_domain" class="form-control"
                    value="<?=$set->wagw_domain?>" />
                <?php } ?>
            </div>
            <div class="form-group wagw" id="cekwa"
                <?php if($set->api_wasap != "wagw"){ echo 'style="display:none"'; } ?>>
            ...
            <div class="form-group text-danger" id="cekwanotif" style="display:none">
                <i class="fas fa-info-circle"></i> Simpan terlebih dahulu sebelum Anda bisa melakukan Scan Kode QR dan
                Cek Koneksi
            </div>
            ...
            <div class="form-group">
                ...
                <?php if($this->func->demo() == true){ ?>
                <input type="text" name="rajaongkir" class="form-control" value="abcdefghijklmnopqrstuvwxyz1234567890"
                    <?=$read?> />
                <?php }else{ ?>
            ...
            <div class="form-group">
                ...
                <?php if($this->func->demo() == true){ ?>
                <input type="text" name="fcm_serverkey" class="form-control"
                    value="abcdefghijklmnopqrstuvwxyz1234567890" <?=$read?> />
                <?php }else{ ?>
            ...
            <div class="form-group">
                ...
                <?php }else{ ?>
                <input type="text" name="google_client_secret" class="form-control"
                    value="<?=$set->google_client_secret?>" />
                <?php } ?>
            </div>
        </div>
...
function cekWA() {
    swal.fire("Tidak dapat terhubung", "Masukkan dulu nomer sender dan domain/subdomain server WA CctvGrosir", "error");
}
<?php }else{ ?>
...
socket.on('qr', function(src) {
    console.log(src)
    $(`#cardimg-${src.id}`).html(`<img src="` + src.src +
        `" class="card-img-top" alt="cardimg" id="qrcode" style="height:250px; width:250px;">`);
    $(`.info-detik-${src.id}`).html(
        `<p class='text-center'>Waktu scan anda adalah <span class="text-primary fs-20">13</span> detik</p>`
        );
    var count = 0;
    var interval = setInterval(function() {
        ...
        counts = 13 - count;
        $(`.info-${src.id}`).html(
            `<p class='text-center'>sisa waktu scan <span class="text-danger fs-24">${counts}</span> detik lagi</p>`
            );
        if (count == 13) {
            $(`.info-${src.id}`).html("");
            $(`#cardimg-${src.id}`).html(
                `<h5 class="text-center text-warning mt-4 m-b-12">Silahkan tutup dulu jendela ini lalu klik tombol scan lagi untuk scan ulang<h5>`
                );
