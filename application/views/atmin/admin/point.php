...
function loadVoucher(page) {
    ...
    $.post("<?=site_url("api/logpoint?load=true&page=")?>" + page, {
        "cari": $("#cari").val(),
...
function dcc(id) {
    swal.fire({
        text: "data yang sudah ditolak tidak dapat dikembalikan lagi",
        ...
        if (vals.value) {
            $.post("<?=site_url("api/dccrequest")?>", {
                ...
                if (data.success == true) {
                    loadVoucher(1);
                    swal.fire("Berhasil", "Aksi berhasil dilakukan", "success");
                } else {
                    swal.fire("Gagal!", "Aksi Gagal, coba ulangi beberapa saat lagi", "error");
                }
            });
        }
    });
...
function acc(id) {
    swal.fire({
        text: "data yang sudah diterima tidak dapat dikembalikan lagi",
        title: "Yakin menerima data ini?",
        type: "info",
        showCancelButton: true,
        cancelButtonColor: "#fffff",
        cancelButtonText: "Batal"
...
function hapus(id) {
    swal.fire({
        ...
        if (vals.value) {
            $.post("<?=site_url("api/hapusvoucher")?>", {
                "id": id,
                [$("#names").val()]: $("#tokens").val()
            }, function(msg) {
                var data = eval("(" + msg + ")");
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="voucherform">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="kode" name="kode" class="form-control col-md-6 kodevoucher text-danger"
                            required />
                    </div>
                    <div class="form-group row m-lr-0">
                        <label class="col-12">Masa Berlaku</label>
                        <div class="col-md-6 p-lr-5">
                            <input type="text" id="mulai" name="mulai" class="form-control m-b-10 dtp"
                                placeholder="tahun-bulan-tanggal" value="" required />
                        </div>
                        <div class="col-md-6 p-lr-5">
                            <input type="text" id="selesai" name="selesai" class="form-control m-b-10 dtp"
                                placeholder="tahun-bulan-tanggal" value="" required />
                        </div>
                    </div>
                    <div class="form-group">
            ...
            */
            ...
            <div class="form-group">
                ...
                <input type="number" id="peruser" name="peruser" value="1" min="1" class="form-control col-md-2"
                    required />
            </div>
