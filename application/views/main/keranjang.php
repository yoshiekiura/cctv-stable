    ...
    <form action="<?=site_url("checkout")?>" method="POST">
            ...
            <div class="keranjang row" id="produk_<?php echo $car->id; ?>">
                <div class="col-md-1 col-4 pointer">
                    <input type="checkbox" class="pointer cebox" name="idproduk[]" value="<?=$car->id?>"
                        id="cebox_<?=$car->id?>" data-total="<?=$car->harga*$car->jumlah?>" />
                </div>
                <div class="col-md-2 col-4 pointer">
                    <div class="img"
                        style="background-image:url('<?php echo $this->func->getFoto($produk->id,"utama"); ?>')"
                        onclick="window.location.href='<?php echo site_url('produk/'.$produk->url); ?>'"></div>
                </div>
                <div class="col-md-9 col-12 row m-lr-0">
                    <div class="col-md-5 m-b-10 centered flex-column pointer"
                        onclick="window.location.href='<?php echo site_url('produk/'.$produk->url); ?>'">
                    ...
                    <div class="col-md-3 col-12 m-b-20 centered">
                        <div class="wrap-num-product input-group">
                            <div class="num-product-down input-group-prepend cursor-pointer">
                                <span class="input-group-text btn-primary"><i
                                        class="fs-16 fas fa-minus text-light"></i></span>
                            </div>

                            <input class="form-control text-center num-product produk-jumlah" type="number"
                                min="<?php echo $produk->minorder; ?>" id="jumlah_<?php echo $car->id; ?>"
                                name="jumlah[]" value="<?php echo $car->jumlah; ?>"
                                data-proid="<?php echo $car->id; ?>" />

                            <div class="num-product-up input-group-append cursor-pointer">
                                <span class="input-group-text btn-primary"><i
                                        class="fs-16 fas fa-plus text-light"></i></span>
                            </div>
                        </div>
                    </div>
                    <div class="col-8 col-md-3 centered">
                        Rp&nbsp;<span
                            id="totalhargauang_<?php echo $car->id; ?>"><?php echo $this->func->formUang($car->harga*$car->jumlah); ?></span>
                    </div>
                    <div class="col-4 col-md-1 centered">
                        <a href="javascript:void(0)" onclick="hapus(<?=$car->id?>)" class="btn btn-danger"><i
                                class="fa fa-trash-alt"></i></a>
                    </div>
                </div>
            </div>
            <input type="hidden" id="harga_<?php echo $car->id; ?>" value="<?php echo $car->harga; ?>" />
            <input type="hidden" class="totalhargaproduk" id="totalharga_<?php echo $car->id; ?>"
                value="<?php echo $car->harga*$car->jumlah; ?>" />
        ...
        <div class="p-t-18 p-b-15 p-lr-40 p-lr-15-sm m-b-90">
            <div class="row">
                ...
                <div class="col-md-4">
                    <h5 class="m-b-20 font-bold text-success text-right">Total : Rp <span id="totalbayar"
                            style="padding-left:40px"><?php echo $this->func->formUang($totalbayar); ?></span></h5>
                </div>
            </div>
            <div class="row">
                    ...
                    if (willDelete.isConfirmed) {
                        ...
                        $.post("<?php echo site_url("assync/hapuskeranjang"); ?>", {
                            ...
                            if (data.success == true) {
                                ...
                                swal.fire("Gagal",
                                    "Gagal menghapus pesanan, silahkan ulangi beberapa saat lagi",
                                    "error");
                            }
                        });
                    } else {
